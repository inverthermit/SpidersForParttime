...
public class NewcastleUnData {
   ...
   */
	...
	{
		File outputFile = new File(FILE_PATH + "\\" + "gen_databefore100.xls");
		...
		for(;i<getURL.UnData.length+1;i++)
		{
			if(i>=0)
			{
				System.out.println(i+":"+getURL.UnData[i-1][0]);
					HashMap<String,String> data=NewCastleGetDetails(getURL.UnData[i-1]);
					for(j=0;j<13;j++)
						{
	...
	public static HashMap<String,String> NewCastleGetDetails(String[] url) throws Exception
	{
        ...
        //   entryrequirements/
        httpGet = new HttpGet(url[0].replace("modules/", "entryrequirements/")); 
        ...
        //<div class='expandable'>
  	    ...
  	    AndFilter EntryFilter=new AndFilter(new TagNameFilter("div"),
                  new HasAttributeFilter("class","expandable"));
          ...
          if(entryAll.equals(""))
          {
              ...
              if(nodesE.size()>0)
              {
              	for(int i=0;i<nodesE.size();i++)
          	    {
          	    	...
          	    	entryAll=(html2Str(node.toHtml())).replace("\r", "");
          	    	entryAll=entryAll.replace("\n", " ");
          	    	//entry=entry.replace("<br>", " ");
        ...
        //finance/
        httpGet = new HttpGet(url[0].replace("modules/", "finance/")); 
  		response = httpclient.execute(httpGet);  
  		entity = response.getEntity();
