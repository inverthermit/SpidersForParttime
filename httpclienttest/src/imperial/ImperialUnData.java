...
public class ImperialUnData {
		...
		for(;i<getURL.UnData.length+1;i++)
		{
			//if(i>=239)
			{
				System.out.println(i+":"+getURL.UnData[i-1][0]);
					HashMap<String,String> data=ImperialGetDetails(getURL.UnData[i-1],i-1);
					for(j=0;j<13;j++)
						{
							//label = new Label(j, i, data.get(Keys[j]));
						    label = new Label(j, Integer.parseInt(data.get("index"))+1, data.get(Keys[j]));
							sheet.addCell(label);
						}
	...
	public static HashMap<String,String> ImperialGetDetails(String[] url,int index)
	{
		while(true)
		{
			try{
