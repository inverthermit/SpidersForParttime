...
public class BirminghamPostData {
	...
	{
		File outputFile = new File(FILE_PATH + "\\" + "BirminghamPost.xls");
		...
		int j=0;
		String url="";
		for(;i<getPostURL.PostData.length+1;i++)
		{
			if(i<=12)
			{
				System.out.println(i+":"+getPostURL.PostData[i-1][0]);
					HashMap<String,String> data=MaryGetDetails(getPostURL.PostData[i-1]);
					for(j=0;j<13;j++)
						{
	...
	public static HashMap<String,String> MaryGetDetails(String[] url) throws Exception
	{
      ...
      //**************************get entry**********************
        ...
        if(nodes4.size()>=4)
        {
    	    	...
    	    	Node node=(Node)nodes4.elementAt(3);
                ...
                result.put("Tuition Fee",fee);
                
                node=(Node)nodes4.elementAt(1);
